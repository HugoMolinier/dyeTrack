...
public class ExerciseService implements ExerciseUseCase {
    ...
    public List<Exercise> getAll(String name, Boolean officialExercise, Long idUser, Boolean onlyPrincipalMuscle,Boolean showMainFocusMuscularGroup,List<Integer> idsGroupeMuscle,List<Integer> idMuscle,List<Long> idsExercice){
        if (showMainFocusMuscularGroup || idsGroupeMuscle!=null){
            return exercisePort.getAllWithShowGroupe( name,  officialExercise,  idUser,onlyPrincipalMuscle,idsGroupeMuscle,idMuscle,idsExercice);
        }
    ...
    @Transactional
    public void create(String nameFR, String description, String linkVideo, Long idUser,List<MuscleInsertExercice> relExerciseMuscles) {
        ...
        for(MuscleInsertExercice muscleInfo : relExerciseMuscles){
            ...
            if(existingPrincipal == null){
                ...
                if(muscle == null) throw new ExerciseCreationException("muscle Not found with id " + muscleId);
    ...
    @Transactional
    public Exercise update(Long idExercice,Long idUserQuiModifie ,String nameFR, String description, String linkVideo,List<MuscleInsertExercice> relExerciseMuscles) {
        if (idExercice == null) throw new ExerciseCreationException("idExercice empty");
        Exercise exercise = exercisePort.getByIdExercise(idExercice);
        if(exercise == null) throw new ExerciseCreationException("exercise Not found with id "+ idExercice);
        if (exercise.getUser() == null) throw new ExerciseCreationException("Impossible de modifier un exercice officiel");
        if (!exercise.getUser().getId().equals(idUserQuiModifie)) throw new ExerciseCreationException("Cet utilisateur ne peut pas modifier cet exercice");
    ...
    public void delete(Long idExercice, Long idUserQuiDelete) {
        ...
        Exercise exercise = exercisePort.getByIdExercise(idExercice);
        if(exercise == null) throw new ExerciseCreationException("exercise Not found with id "+ idExercice);
        if (exercise.getUser() == null) throw new ExerciseCreationException("Impossible de delete un exercice officiel");
        if (!exercise.getUser().getId().equals(idUserQuiDelete)) throw new ExerciseCreationException("Cet utilisateur ne peut pas delete cet exercice");
