...
public class UserService implements UserUseCase {
    ...
    @Transactional
    public AuthValue save(String pseudo, String email, String password,LocalDate dateNaissance, Integer taille, Boolean sexeMale){
        if (pseudo == null || pseudo.isBlank()) throw new ForbiddenException("pseudo empty");
        if (email == null || email.isBlank()) throw new ForbiddenException("email empty");
        if (password == null || password.isBlank()) throw new ForbiddenException("password empty");
        if (taille != null && (taille < 50 || taille > 300)) throw new ForbiddenException("taille Not realistic");
        ...
        if(userVerif != null) throw new ForbiddenException("Email déjà utilisé : "+ email);
        String hashedPassword = passwordEncoder.encode(password);
        java.sql.Date sqlDateNaissance = dateNaissance != null ? java.sql.Date.valueOf(dateNaissance) : null;

        User user = userPort.save(new User(pseudo, hashedEmail, hashedPassword, new Date(System.currentTimeMillis()), sqlDateNaissance, taille, sexeMale));
        return new AuthValue(jwtService.generateToken(user.getId()), user);
    }

    @Transactional
    public AuthValue login( String email, String password){
        if (email ==null) throw new ForbiddenException("email empty");
        if (password ==null) throw new ForbiddenException("password empty");
         if (email == null || email.isBlank()) throw new ForbiddenException("email empty");
        if (password == null || password.isBlank()) throw new ForbiddenException("password empty");
        String hashedEmail = HashUtil.hashEmail(email, emailSecretKey);
        User user = userPort.findByMailHashed(hashedEmail);
        if (user == null || !passwordEncoder.matches(password, user.getPassword())) throw new ForbiddenException("wrong password or email");
        return new AuthValue(jwtService.generateToken(user.getId()), user);
    }
    ...
    public User update(Long idTokenUser, Long idUser, String pseudo, String password,
        ...
        if (!idTokenUser.equals(idUser)) throw new ForbiddenException("Not same user");
    
        User user = userPort.get(idUser);
        if (user == null) throw new ForbiddenException("User not found");
        if (pseudo != null && !pseudo.isBlank()) user.setPseudo(pseudo);
        if (password != null && !password.isBlank()) user.setPassword(passwordEncoder.encode(password));
        if (dateNaissance != null) user.setDateNaissance(java.sql.Date.valueOf(dateNaissance));
        ...
        if (sexeMale != null) user.setSexeMale(sexeMale);
