...
public class PresetSeanceService implements PresetSeanceUseCase {
    ...
    public PresetSeanceService(PresetSeancePort presetSeancePort,UserPort userPort,LateralitePort lateralitePort,EquipementPort equipementPort,ExercisePort exercisePort,PresetSeanceExercicePort presetSeanceExercicePort){
    ...
    public PresetSeance save(String name, Long idUserWhoAdd,List<PresetSeanceExerciceVO> presetSeanceExercice){
        if(name == null) throw new ExerciseCreationException("name cannot be empty");
        User user = userPort.get(idUserWhoAdd);
        if(user == null) throw new ExerciseCreationException("user Not found with id "+ idUserWhoAdd);
        PresetSeance presetSeance = presetSeancePort.save(new PresetSeance(name,user));

        //do relExerciceMuscle

        int index = 1; // compteur pour orderExercice
    ...
    public PresetSeance update(Long idPreset,Long idUserQuiModifie, String newName, List<PresetSeanceExercice> relExerciseMuscles){
        if (idPreset == null) throw new ForbiddenException("idPreset empty");
        PresetSeance presetSeance = presetSeancePort.getById(idPreset);
        if(presetSeance == null) throw new ForbiddenException("Preset Not found with id "+ idPreset);
        if (!presetSeance.getUser().getId().equals(idUserQuiModifie)) throw new ForbiddenException("Cet utilisateur ne peut pas modifier ce preset");
    ...
    public void delete(Long idpresetSeance,Long idUserQuiModifie){
        if (idpresetSeance == null) throw new ForbiddenException("idPreset empty");
        PresetSeance presetSeance = presetSeancePort.getById(idpresetSeance);
        if(presetSeance == null) throw new ForbiddenException("Preset Not found with id "+ idpresetSeance);
        if (!presetSeance.getUser().getId().equals(idUserQuiModifie)) throw new ForbiddenException("Cet utilisateur ne peut pas delete ce preset");
